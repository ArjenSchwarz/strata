...
func TestHasDangerIndicator(t *testing.T) {
	...
	tests := []struct {
		...
		// Test explicit danger indicators
		{
			name:     "detects warning emoji",
			row:      "| Add | aws_instance.example | - | ⚠️ Sensitive resource |",
			expected: true,
		},
		{
			name:     "detects Sensitive keyword",
			row:      "| Remove | aws_db.main | - | Sensitive database |",
			expected: true,
		},
		{
			name:     "detects Dangerous keyword",
			row:      "| Replace | aws_instance.prod | - | Dangerous change |",
			expected: true,
		},
		{
			name:     "detects High Risk keyword",
			row:      "| Modify | aws_security_group.main | - | High Risk modification |",
			expected: true,
		},
		{
			name:     "detects Critical keyword",
			row:      "| Replace | aws_rds_cluster.main | - | Critical infrastructure |",
			expected: true,
		},
		// Test danger column detection
		{
			name:     "detects non-empty danger column",
			row:      "| Add | aws_instance.test | instance_type | Security warning |",
			expected: true,
		},
		{
			name:     "ignores dash in danger column",
			row:      "| Add | aws_instance.test | instance_type | - |",
			expected: false,
		},
		{
			name:     "ignores empty danger column",
			row:      "| Add | aws_instance.test | instance_type |  |",
			expected: false,
		},
		{
			name:     "handles row without danger column",
			row:      "| Add | aws_instance.test |",
			expected: false,
		},
		// Edge cases
		{
			name:     "handles malformed row with single column",
			row:      "| Add |",
			expected: false,
		},
		{
			name:     "handles row with trailing spaces",
			row:      "| Add | aws_instance.test | - | Warning   |   ",
			expected: true,
		},
		{
			name:     "handles row with multiple pipes",
			row:      "| Add | aws_instance.test | prop1 | prop2 | Some danger text |",
			expected: true,
		},
		{
			name:     "detects danger in middle columns",
			row:      "| Remove | aws_db.main | Sensitive data | - |",
			expected: true,
		},
		// Ambiguous cases
		{
			name:     "handles mixed indicators",
			row:      "| Replace | aws_instance.prod | - | ⚠️ High Risk Sensitive change |",
			expected: true,
		},
		{
			name:     "normal row without dangers",
			row:      "| Add | aws_instance.test | instance_type | - |",
			expected: false,
		},
	}
...
// TestActionSortTransformer_Transform tests the enhanced table sorting
// Task 6.2 from Output Refinements feature
func TestActionSortTransformer_Transform(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "sorts by danger first, then action priority",
			format: "table",
			input: `### Resource Changes

| action | resource | properties | danger |
| --- | --- | --- | --- |
| Add | aws_instance.test | instance_type | - |
| Remove | aws_db.main | - | ⚠️ Sensitive database |
| Replace | aws_instance.prod | ami | - |
| Modify | aws_security_group.main | rules | High Risk modification |
...
### Summary`,
		...
		{
			name:   "sorts alphabetically when same danger and action",
			format: "table",
			input: `### Resource Changes

| action | resource | properties | danger |
| --- | --- | --- | --- |
| Add | aws_instance.zulu | instance_type | - |
| Add | aws_instance.alpha | instance_type | - |
| Add | aws_instance.charlie | instance_type | - |`,
			expected: `### Resource Changes
...
| Add | aws_instance.zulu | instance_type | - |`,
		...
		{
			name:   "handles mixed danger levels and actions",
			format: "table",
			input: `### Resource Changes

| action | resource | properties | danger |
| --- | --- | --- | --- |
| Add | aws_s3_bucket.logs | - | - |
| Remove | aws_instance.web1 | - | - |
| Replace | aws_db_instance.main | - | Critical infrastructure |
| Modify | aws_iam_role.admin | - | ⚠️ Sensitive |
| Remove | aws_db_instance.backup | - | Sensitive database |`,
			expected: `### Resource Changes
...
| Add | aws_s3_bucket.logs | - | - |`,
		...
		{
			name:   "handles table without danger column",
			format: "table",
			input: `### Resource Changes
...
| Remove | aws_db.main | - |`,
		...
		{
			name:   "ignores non-resource tables",
			format: "table",
			input: `### Statistics
...
| Add | 2 |`,
		...
		{
			name:   "handles filtered content correctly",
			format: "table",
			input: `### Resource Changes

| action | resource | properties | danger |
| --- | --- | --- | --- |
| Replace | aws_instance.prod | ami | High Risk |
| Add | aws_instance.test | type | - |`,
			expected: `### Resource Changes
...
| Add | aws_instance.test | type | - |`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := transformer.Transform(ctx, []byte(tt.input), tt.format)
			if err != nil {
...
// TestActionSortTransformer_extractResourceAddress tests resource address extraction
func TestActionSortTransformer_extractResourceAddress(t *testing.T) {
	transformer := &ActionSortTransformer{}
	...
	tests := []struct {
		...
		{
			name:     "extracts from standard row",
			row:      "| Add | aws_instance.example | instance_type | - |",
			expected: "aws_instance.example",
		},
		{
			name:     "handles row with fewer columns",
			row:      "| Add | aws_instance.test |",
			expected: "aws_instance.test",
		},
		{
			name:     "handles empty resource column",
			row:      "| Add |  | instance_type | - |",
			expected: "",
		},
		{
			name:     "handles malformed row",
			row:      "| Add |",
			expected: "",
		},
		{
			name:     "trims whitespace",
			row:      "| Add |   aws_instance.test   | type | - |",
			expected: "aws_instance.test",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := transformer.extractResourceAddress(tt.row)
			if result != tt.expected {
				t.Errorf("extractResourceAddress() = %q, expected %q", result, tt.expected)
			}
		})
	}
}

// TestActionSortTransformer_getActionPriority tests action priority mapping
func TestActionSortTransformer_getActionPriority(t *testing.T) {
	transformer := &ActionSortTransformer{}
	...
	for _, tt := range tests {
		t.Run(tt.action, func(t *testing.T) {
			result := transformer.getActionPriority(tt.action)
			if result != tt.expected {
...
// TestActionSortTransformer_extractAction tests action extraction
func TestActionSortTransformer_extractAction(t *testing.T) {
	transformer := &ActionSortTransformer{}
	...
	tests := []struct {
		...
		{
			name:     "extracts Remove action",
			row:      "| Remove | aws_db.main | - |",
			expected: "Remove",
		},
		{
			name:     "extracts Replace action",
			row:      "| Replace | aws_instance.prod | ami |",
			expected: "Replace",
		},
		{
			name:     "extracts Add action",
			row:      "| Add | aws_instance.test | instance_type |",
			expected: "Add",
		},
		{
			name:     "extracts Modify action",
			row:      "| Modify | aws_sg.main | rules |",
			expected: "Modify",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := transformer.extractAction(tt.row)
			if result != tt.expected {
				t.Errorf("extractAction(%q) = %q, expected %q", tt.row, result, tt.expected)
			}
		})
	}
}
