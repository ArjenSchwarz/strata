...
func TestRiskBasedSorting_Integration(t *testing.T) {
	...
	// Configure sensitive resources to ensure dangerous items are flagged
	cfg.SensitiveResources = []config.SensitiveResource{
		{ResourceType: "aws_db_instance"},
		{ResourceType: "aws_rds_db_instance"},
		{ResourceType: "aws_s3_bucket"},
	...
	// Create formatter and test different output formats
	formatter := NewFormatter(cfg)
	...
	testFormats := []string{"table", "markdown", "html", "json"}

	for _, format := range testFormats {
		t.Run(format+"_sorting", func(t *testing.T) {
			outputConfig.Format = format

			// Test that ActionSortTransformer is applied to supported formats
			actionSortTransformer := &ActionSortTransformer{}
			formatObj := formatter.getFormatFromConfig(format)

			if actionSortTransformer.CanTransform(formatObj.Name) {
				t.Logf("ActionSortTransformer supports format: %s", format)

				// Test the transformer can handle the format
				err := formatter.OutputSummary(summary, outputConfig, true)
				if err != nil {
					t.Errorf("Failed to output summary for format %s: %v", format, err)
				}
			} else {
				t.Logf("ActionSortTransformer does not support format: %s", format)
			}
		})
	}

	// Specifically test sorting order in data before formatting
	// Dangerous items should come first, then sorted by action type (delete, replace, update, add)
	dangerousChanges := make([]ResourceChange, 0)
	nonDangerousChanges := make([]ResourceChange, 0)

	for _, change := range summary.ResourceChanges {
		// Skip no-op changes as they should be hidden
	...
	// Verify we have both dangerous and non-dangerous changes for a proper test
	if len(dangerousChanges) == 0 {
		t.Error("Expected dangerous changes for sorting test")
	}
	if len(nonDangerousChanges) == 0 {
		t.Error("Expected non-dangerous changes for sorting test")
	}

	// Test that no-op changes are filtered out
	noOpCount := 0
	for _, change := range summary.ResourceChanges {
		if change.ChangeType == ChangeTypeNoOp {
			noOpCount++
	...
	if noOpCount > 0 {
		t.Logf("Found %d no-op changes that should be filtered from output", noOpCount)
	}

	// Test sorting consistency - the ActionSortTransformer should work in post-processing
	// so we mainly verify that it can process the output without errors
	actionSortTransformer := &ActionSortTransformer{}

	// Test with table format
	tableFormat := formatter.getFormatFromConfig("table")
	if actionSortTransformer.CanTransform(tableFormat.Name) {
		t.Log("Verified ActionSortTransformer can process table format")
	}

	// Test with markdown format
	markdownFormat := formatter.getFormatFromConfig("markdown")
	if actionSortTransformer.CanTransform(markdownFormat.Name) {
		t.Log("Verified ActionSortTransformer can process markdown format")
	}

	// Test with HTML format
	htmlFormat := formatter.getFormatFromConfig("html")
	if actionSortTransformer.CanTransform(htmlFormat.Name) {
		t.Log("Verified ActionSortTransformer can process HTML format")
	}

	// Test with CSV format
	csvFormat := formatter.getFormatFromConfig("csv")
	if actionSortTransformer.CanTransform(csvFormat.Name) {
		t.Log("Verified ActionSortTransformer can process CSV format")
	}
}

// TestActionSortTransformer_FormatSupport tests that ActionSortTransformer supports the expected formats
func TestActionSortTransformer_FormatSupport(t *testing.T) {
	transformer := &ActionSortTransformer{}

	// Test that transformer supports the formats as defined in its CanTransform method
	expectedSupported := []string{"table", "markdown", "html", "csv"}
	expectedNotSupported := []string{"json"}

	for _, format := range expectedSupported {
		if !transformer.CanTransform(format) {
			t.Errorf("Expected ActionSortTransformer to support format '%s', but it doesn't", format)
		}
	}

	for _, format := range expectedNotSupported {
		if transformer.CanTransform(format) {
			t.Errorf("Expected ActionSortTransformer to NOT support format '%s', but it does", format)
		}
	}

	// Additionally test that transformer correctly identifies unsupported formats
	unsupportedFormats := []string{"yaml", "xml", "txt"}
