...
### Fixed
- **Action Foundation Test Sourcing**: `make test-action-foundation` failed because `action.sh` ran `main "$@"` unconditionally at the bottom, so the test harness's `source_script` (which sources the script to inspect its functions) triggered input validation and exited with `Plan file is required`. The final `main` invocation is now guarded with `if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then ... fi` so it runs only on direct execution (the GitHub Action still invokes it directly). The foundation test was also repointed at `action_simplified.sh` (the script it exercises) and several latent test defects exposed once the early exit was removed were fixed: `run_function` now captures `exit`-based status, `source_script` strips the EXIT trap, `csv` is treated as a valid output format, `log_error` output capture no longer aborts under `set -e`, and the `wc -l` directory count is normalised for BSD/macOS.
